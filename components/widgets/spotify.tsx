...
export const SpotifyWidget = () => {
  ...
  // let spotifyCodeFromURL;
  // const [spotifyCodeFromURL, setSpotifyCodeFromURL] = useState(null);
  ...
  useEffect(() => {
    getSpotifyData();
    // spotifyCodeFromURL = new URLSearchParams(window.location.search).get('code');
    // when a user first lands here, i need to alrady have the spotify data
    // window.localStorage.setItem('spotify_code', spotifyCodeFromURL);

    // TODO: move this
    // refreshAccessToken();
    // getSpotifyData();

    // TODO: Fetch every 5 mins?
  ...
  //   window.location.href = encodeURI(`https://accounts.spotify.com/authorize?client_id=${CLIENT_ID}&response_type=${RESPONSE_TYPE}&redirect_uri=${REDIRECT_URI}&scope=${SCOPE}`)
  // }

  // const requestFirstAccessTokens = async () => {
  //   const GRANT_TYPE = 'authorization_code'
  //   const CODE = spotifyCodeFromURL
  ...
  //   await axios.post(ENDPOINT, querystring.stringify(data), { headers: headers })
  //     .then(response => {
  ...
  //       console.log('requested new access token and set tokens to local stoage', response);
  //     })
  // }
  ...
  //     .then(response => {
  //       console.log('this actually works fuck yeah!', response);
  //     })
  // }
  ...
  //   await axios.get(endpoint + 'currently-playing', {
  //     headers: {
  //       "Authorization": `Bearer ${window.localStorage.getItem('spotify_access_token')}`
  //     }
  //   }).then(response => {
  //     // not online! fetch recently played
  //     if (response.status === 204) {
  //       fetchRecentlyPlayed();
  //     } else {
  //       setSpotifyData({
  //         item: response.data.item,
  //         status: response.data.is_playing
  //       });
  //     }
  //   })
  // }
  ...
  //   await axios.get(endpoint + 'recently-played', {
  //     headers: {
  //       "Authorization": `Bearer ${window.localStorage.getItem('spotify_access_token')}`
  //     }
  //   }).then(response => {
  ...
  return (
    <>
      ...
      <button onClick={() => requestFirstAccessTokens()}>Request Both Acc+Refr. Tokens from Spotify</button>
      <button onClick={() => refreshAccessToken()}>Refresh Access Token from Spotify</button>
      <button onClick={() => getSpotifyData()}>Get Spotify Data :D</button> */}
      <>
